 ...
 */
    ...
    public static SIPMessage parseMessage(String message) throws SIPException{
        ...
        if(message.startsWith("INVITE")){
         ...
         // 
         // 1. Usa el Content-Length real del HEADER
            int cl = 0;
            if (contentLength != null) {
                cl = Integer.parseInt(contentLength);
            }
            ok.setContentLength(cl); // Usar el valor parseado

            // 2. Solo parsear SDP si el Content-Length es mayor que 0
            if(cl > 0 && parts.length == 2){
            	//SDP message
            	String sdpMessageStr = parts[1];
            	SDPMessage sdp = new SDPMessage();
            	sdp.parseMessage(sdpMessageStr);
                ...
                // 3. Si no hay SDP, asegurarse de que el objeto sdp sea NULL
                ok.setSdp(null);
            }
